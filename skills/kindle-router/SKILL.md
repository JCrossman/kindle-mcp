---
name: kindle-router
description: Carry out pending Kindle @commands (@post, @research, @todo, @project, @quote) through the kindle MCP server, saving each result with kindle_complete_command. Use when asked to process, route or action Kindle notes, or from a scheduled run.
---
...
3. Call `kindle_get_pending_commands` with `limit` 20. Work through the highlights one at a time, in the order returned.
...
7. After the batch, call `kindle_get_pending_commands` again and list every id still pending with the reason.
